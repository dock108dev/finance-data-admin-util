...
and serves them over HTTP. Isolates Docker socket access into a single
...
def _fetch_logs(container: str, lines: int) -> tuple[int, dict[str, object]]:
    """Fetch logs from Docker Engine API."""
...
class LogRelayHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            ...
            self._respond(503, {"error": f"Docker socket unavailable: {exc}"})
            return
